...
class SyntheticDatasetGenerator:
    ...
    def _resolve_background_dir(self) -> None:
        ...
        """
        cfg = self.config
        if cfg.resources.bg_image_dir:
            return
        if not cfg.resources.auto_download_backgrounds:
            return
        cache_dir = cfg.resources.background_cache_dir or ".background_cache"
        print("No bg_image_dir given - downloading curated background images...")
        downloader = BackgroundDownloader(
            cache_dir=cache_dir,
            manifest_url=cfg.resources.background_manifest_url,
            timeout=cfg.resources.font_download_timeout,
            enabled=True,
        )
        paths = downloader.download_all()
        if paths:
            cfg.resources.bg_image_dir = cache_dir
    ...
    def _prepare_train_val(self) -> tuple[list[str], list[str]]:
        ...
        if cfg.resources.wordlist_path is not None:
            words = DatasetSplitter.load_vocabulary(cfg.resources.wordlist_path)
            print(f"Loaded {len(words)} words from wordlist '{cfg.resources.wordlist_path}'.")
            if charset:
                before = len(words)
                words = filter_in_vocab(words, charset)
                print(f"  vocab restriction: kept {len(words)}/{before} words within the target vocab.")
            train, val = DatasetSplitter.prepare_splits(
                words, cfg.core.num_images, cfg.core.val_percent, ensure_coverage=True
            )
    ...
    def _coverage_target(self, languages: list[str]) -> set[str] | None:
        ...
        cfg = self.config
        if not cfg.coverage.ensure_vocab_coverage:
            return None
        chars: set[str] = set()
        if cfg.coverage.target_vocab:
            explicit = resolve_target_vocab(None, cfg.coverage.target_vocab)
            if explicit:
                chars |= explicit
        else:
            for lang in languages:
    ...
    def _inject_coverage(self, train: list[str], val: list[str], target: set[str]) -> tuple[list[str], list[str]]:
        ...
        train, n_train, miss_train = augment_words_for_coverage(
            train, target, min_count=cfg.coverage.vocab_coverage_min_count, seed=seed
        )
    ...
    def generate_dataset(self):
        ...
        if self.config.core.task == "detection":
            return self._generate_detection_dataset()

        print(f"Generating dataset with {self.config.core.num_workers} workers...")
        self._resolve_background_dir()

        ext = "jpg" if self.config.core.output_jpeg else "png"
        ...
        train_dir = Path(self.config.core.output_dir) / "train"
        val_dir = Path(self.config.core.output_dir) / "val"
    ...
    def _generate_images_with_queue(self, tasks: List[GenerationTask]) -> Tuple[int, Dict[str, str]]:
        ...
        # Create queues
        task_queue: mp.Queue = mp.Queue(maxsize=self.config.core.queue_maxsize)
        result_queue: mp.Queue = mp.Queue()

        # Start worker processes
        workers = []
        for worker_id in range(self.config.core.num_workers):
            worker = mp.Process(
        ...
        for _ in range(self.config.core.num_workers):
            task_queue.put(None)
    ...
    def _resolve_word_pool(self) -> List[str]:
        ...
        if cfg.resources.wordlist_path is not None:
            ...
            words = downloader.build_vocabulary(languages, words_per_language=cfg.corpus.words_per_language)
            if cfg.corpus.casing_variant_prob > 0:
                words = apply_casing_variants(words, prob=cfg.corpus.casing_variant_prob, seed=cfg.corpus.seed)
            if cfg.corpus.numeric_token_ratio > 0:
                words = words + generate_numeric_tokens(
                    int(len(words) * cfg.corpus.numeric_token_ratio), seed=cfg.corpus.seed
                )
        if not words:
            raise ValueError("No words available for detection page generation. Check connectivity or wordlist.")
        random.Random(cfg.corpus.seed).shuffle(words)
        return words

    def _generate_detection_dataset(self):
        """Generate document-like pages with per-word polygons (docTR format)."""
        cfg = self.config
        print(f"Generating DETECTION dataset with {cfg.core.num_workers} workers...")
        self._resolve_background_dir()
        ext = "jpg" if cfg.core.output_jpeg else "png"

        pool = self._resolve_word_pool()
        rng = random.Random(cfg.corpus.seed)

        num_val = math.ceil(cfg.core.num_images * cfg.core.val_percent)
        num_train = max(0, cfg.core.num_images - num_val)
        print(f"Generating {num_train} training pages and {num_val} validation pages.")

        train_dir = Path(cfg.core.output_dir) / "train"
        val_dir = Path(cfg.core.output_dir) / "val"
        train_images_dir = train_dir / "images"
        val_images_dir = val_dir / "images"
        ...
        def make_tasks(count: int, images_dir: Path, split: str):
            ...
            for idx in range(count):
                page_words = [rng.choice(pool) for _ in range(cfg.detection.max_words_per_page)]
                yield DetectionTask(
                    words=page_words,
